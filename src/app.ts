...
interface UsageData {
  generatedAt: string;
  pricing?: { source: "live" | "static"; fetchedAt: number | null };
  rows: UsageRow[];
...
function initialRange(): number | "all" {
...
function baseModelLabel(id: string): string {
...
function seriesColor(model: string): string {
  if (model === OTHER) return cssVar("--text-muted");
  const slot = state.slots.get(model);
  return slot != null ? cssVar(SLOT_VARS[slot]) : cssVar("--text-muted");
}

function totalTokens(r: UsageRow): number {
...
function assignSlots(rows: UsageRow[]): void {
  ...
  for (const m of ordered) {
    if (state.slots.has(m)) continue;
    if (state.slots.size >= 8) break;
    state.slots.set(m, state.slots.size);
  }
}

function inRange(dateStr: string, from: string | null): boolean {
...
function rangeStart(): string | null {
  if (state.rangeDays === "all") return null;
  const d = new Date();
  d.setDate(d.getDate() - (state.rangeDays - 1));
  return localISO(d);
}

function foldModel(model: string): string {
...
function renderTiles(rows: UsageRow[], sessions: Array<{ date: string; count: number }>): void {
  ...
  let todaySub: string;
  if (todayCost === 0) todaySub = "nothing recorded yet";
  else if (avgPerDay <= 0 || dayCount < 2) todaySub = "first day in range";
  else {
...
function buildDays(rows: UsageRow[]): string[] {
...
function setEmpty(wrapId: string, className: string, message: string | null): void {
  const wrap = $(wrapId);
...
function renderChart(rows: UsageRow[]): void {
  ...
  const maxTotal = Math.max(
    ...days.map((d) => [...perDay.get(d)!.values()].reduce((a, v) => a + v, 0)),
    0.01,
  );
  ...
  days.forEach((day, i) => {
    ...
    const stack = series.map((m) => [m, dm.get(m) ?? 0] as const).filter(([, v]) => v > 0);
    ...
    hit.addEventListener("pointermove", (ev) => {
      ...
      showTooltip(ev as PointerEvent, day, dm, series);
    });
...
function showTooltip(ev: PointerEvent, day: string, dm: Map<string, number>, series: string[]): void {
  const tt = $("tooltip");
  tt.innerHTML = "";
...
function placeTooltip(tt: HTMLElement, ev: PointerEvent): void {
  ...
  if (x + tt.offsetWidth > window.innerWidth - 8) x = ev.clientX - tt.offsetWidth - 14;
  if (yPos + tt.offsetHeight > window.innerHeight - 8) yPos = ev.clientY - tt.offsetHeight - 14;
  tt.style.left = `${x}px`;
...
function renderDonut(rows: UsageRow[]): void {
  ...
  if ((totals.get(OTHER) ?? 0) > 0) series.push(OTHER);
  ...
  const attachTip = (node: SVGElement, name: string, value: number): void => {
    node.addEventListener("pointermove", (ev) => {
      svg.classList.add("dim");
      showDonutTip(ev as PointerEvent, name, value, total);
    });
...
function showDonutTip(ev: PointerEvent, model: string, value: number, total: number): void {
  ...
  row.append(
    key,
    el("span", "tt-value", fmtCost.format(value)),
    el("span", "tt-name", `${model === OTHER ? OTHER : modelLabel(model)}, ${fmtPct.format(value / total)}`),
  );
  tt.append(row);
  placeTooltip(tt, ev);
}
...
function numCell(text: string, isZero = false): HTMLTableCellElement {
  const td = el("td", "num", text);
  if (isZero) td.classList.add("zero");
  return td;
}

function setTableEmpty(tableId: string, message: string | null): void {
  const table = $(tableId);
...
function renderTables(rows: UsageRow[]): void {
  ...
  const byModel = new Map<string, { cost: number; output: number; cacheRead: number }>();
  for (const r of rows) {
...
function paceText(l: Limit): { text: string; warn: boolean } {
  if (l.percent <= 0) return { text: "Nothing used in this window yet", warn: false };
  if (l.percent >= 100) return { text: "Limit reached, waiting for the reset", warn: true };
  if (l.pacePerHour == null) {
    return { text: l.isSession ? "Measuring pace" : "Pace available after 12 h of history", warn: false };
  }
  if (l.pacePerHour <= 0.01) return { text: "Idle, no recent usage", warn: false };
  ...
  if (l.projectedAtReset == null) return { text: avg, warn: false };
...
function renderLimitTile(l: Limit): HTMLElement {
  ...
  if (l.note) name.append(el("span", "limit-note", l.note));
...
interface ProviderMeta { icon: string; name: string; vendor: string }
...
function providerError(p: ProviderLimits): string {
  const err = p.error ?? "";
  if (/cooldown/i.test(err)) return "Rate-limited by the provider. Showing the last good values, retrying in a few minutes.";
  if (/HTTP 401|HTTP 403/.test(err)) return "Sign-in expired. Log in again with the provider's own CLI.";
  if (/HTTP 429/.test(err)) return "Rate-limited by the provider. Retrying in a few minutes.";
  if (/timeout|abort/i.test(err)) return "The provider did not answer in time. Retrying.";
...
function renderLimits(data: AllLimitsData | null): void {
  ...
  for (const p of providers) {
    ...
    side.append(name);
    if (meta.vendor) side.append(el("div", "provider-vendor", meta.vendor));

    const tags = el("div", "provider-tags");
    if (p.plan) tags.append(el("span", "tag", p.plan));
    tags.append(el("span", `tag ${p.source === "live" ? "live" : "off"}`, p.source === "live" ? "Live" : "Unavailable"));
    side.append(tags);

    if (p.source !== "live") side.append(el("div", "provider-note err", providerError(p)));
    for (const d of p.details) side.append(el("div", "provider-note", d));
    row.append(side);

    const windows = el("div", "provider-windows");
    if (p.limits.length) {
      for (const l of p.limits) windows.append(renderLimitTile(l));
    } else {
...
async function load(): Promise<void> {
  const [usage, limits] = await Promise.all([
    fetchJson<UsageData>("/api/usage"),
    fetchJson<AllLimitsData | null>("/api/limits/all").catch(() => null),
  ]);
  state.data = usage;
...
async function refreshLimits(): Promise<void> {
  try {
    renderLimits(await fetchJson<AllLimitsData | null>("/api/limits/all"));
  } catch {}
}
...
setInterval(() => {
  if (state.limits && document.visibilityState === "visible") renderLimits(state.limits);
}, 30_000);
document.addEventListener("visibilitychange", () => {
  if (!document.hidden) refreshLimits();
});
...
const THEME_ICON: Record<Theme, string> = { system: "ph-circle-half", light: "ph-sun", dark: "ph-moon" };
const THEME_TITLE: Record<Theme, string> = { system: "Theme: follows system", light: "Theme: light", dark: "Theme: dark" };
...
window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change", () => {
  if (state.theme === "system" && state.data) render();
});
...
function syncRangeButtons(): void {
  document.querySelectorAll<HTMLButtonElement>("#filterRow button").forEach((b) => {

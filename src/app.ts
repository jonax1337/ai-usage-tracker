...
function renderTiles(rows: UsageRow[], sessions: Array<{ date: string; count: number }>): void {
  ...
  $("tiles").innerHTML = tiles
    ...
    .join("");
  document.querySelectorAll<HTMLElement>("#tiles .tile").forEach((el, i) => {
...
function renderChart(rows: UsageRow[]): void {
  ...
  days.forEach((day, i) => {
    ...
    hit.addEventListener("pointermove", (ev) => showTooltip(ev as PointerEvent, day, dm, series));

...
const state = { rangeDays: 30, data: null, slots: new Map() };
...
function renderTiles(rows, sessions) {
  ...
  document.getElementById("tiles").innerHTML = tiles
    .map(([label, value, delta]) => `<div class="tile"><div class="label"></div><div class="value"></div><div class="delta"></div></div>`)
    .join("");
...
function renderChart(rows) {
  ...
  for (let v = 0; v <= yMax + 1e-9; v += step) {
    ...
    t.textContent = fmtCost.format(v);
    svg.append(t);
  }
...
window.addEventListener("resize", () => state.data && renderChart(
  state.data.rows.filter((r) => inRange(r.date, rangeStart()))
));
...
function renderLimits(data) {
  ...
  document.getElementById("limitsMeta").textContent =
    `${data.plan ? data.plan + " · " : ""}${freshness}`;

  const wrap = document.getElementById("limits");
  wrap.innerHTML = "";
  for (const l of data.limits) {
    const state = l.severity !== "normal" || l.percent >= 90 ? "critical" : l.percent >= 70 ? "warning" : "normal";
    ...
    meter.setAttribute("aria-valuenow", l.percent);
    ...
    if (l.pacePerHour == null) {
      ...
      } else if (l.projectedAtReset >= 100) {
        pred.classList.add("pred-warn");
...
events.onopen = () => { state.live = true; if (state.data) setSubtitle(); };
events.onerror = () => { state.live = false; if (state.data) setSubtitle(); };

load().catch((err) => {
  document.getElementById("subtitle").textContent = `Fehler beim Laden: ${err}`;
});
